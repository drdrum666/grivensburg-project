...
function deriveDrunkEffects(player, world) {
    ...
    if (st > 0) {
        ...
        /* ИКОНКА ОБЯЗАТЕЛЬНА. Без неё интерфейс рисовал пустой кружок:
           отличить «эффект висит» от «эффектов нет» было нельзя. Своей
           картинки у опьянения пока нет — берём бадунную, они про одно. */
        out.push({
            id: 'drunk', kind: 'debuff', duration: 'battle',
            name: 'Опьянение', icon: EFFECT_ICONS.hangover,
...
function resolveEnemyAttack(p) {
    ...
    var roll = num(p.roll) + (p.targetShielded ? SHIELD_PENALTY : 0);
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   Снимать нечего, значит нечего и забыть снять. */
var SHIELD_PENALTY  = -5;   /* к броску атаки врага по прикрытому воину */
