...
function computeVitals(p) {
    ...
    var maxResource = kind === 'rage' ? (RAGE_BASE + statSteps(t.str) * RAGE_PER_STEP)
                    /* Настроение: +1 за КАЖДОЕ очко харизмы, а не за ступень.
                       По ступеням оно не двигалось: харизма у не-барда
                       побочная и до первого порога не доходила за всю игру,
                       а лимит на выпивку должен расти вместе с героем. */
                    : kind === 'mood' ? (MOOD_BASE + num(t.cha))
                    : maxMP;
...
function computeCombatValues(p) {
    ...
    /* ЗАЩИТА РАСТЁТ ВДВОЕ МЕДЛЕННЕЕ АТАКИ.

       Было: одна и та же величина шла и в удар, и в защиту — по +1 за
       каждые полные 5 сверх пятёрки. На двадцатке силы воин получал +3 и
       туда, и туда. Защита в этой игре весит больше атаки: на тяжёлой
       парирование срезает урон целиком, поэтому такой рост был щедрым.

       Теперь атака по-прежнему каждые 5, а защита каждые 10. */
    if (cls === 'warrior') {
        classAtk = Math.max(0, Math.floor((num(t.str) - 5) / 5));
        classDef = Math.max(0, Math.floor((num(t.str) - 5) / 10));
    }
    /* У ЛУЧНИКА ПРИБАВКИ К АТАКЕ ОТ ЛОВКОСТИ БОЛЬШЕ НЕТ.

       У всех классов из главного стата вычиталась стартовая пятёрка —
       прибавка считалась от того, что герой набрал СВЕРХ начального. У
       лучника вычитания не было, и он получал +1 к атаке просто за то, что
       родился с ловкостью 5. Это выбивалось из ряда, где четыре другие
       формулы написаны одинаково.

       Ловкость теперь даёт лучнику своё: инициативу (как всем) и порог
       крита в режиме разбойника. К атаке она не относится. */
    else if (cls === 'archer') { classAtk = 0; }
    else if (cls === 'mage') { classAtk = Math.max(0, Math.floor((num(t.int) - 5) / 5)); }
    else if (cls === 'priest') {
        classAtk = Math.max(0, Math.floor((num(t.int) - 5) / 5));
        classDef = Math.max(0, Math.floor((num(t.int) - 5) / 10));
    }
    ...
    /* Порог крита. Классовый −1 разбойнику даётся СРАЗУ за два кинжала.
       ...
       просто так. */
    /* ПОРОГ КРИТА — ЭТО ПРО РАЗБОЙНИКА, А НЕ ПРО ЛУЧНИКА.

       Разбойник получает +1 СРАЗУ за два кинжала и ещё по +1 за каждые
       ПОЛНЫЕ 10 ловкости. Лучник с луком от ловкости крит не получает
       вовсе: его размен — инициатива, он ходит раньше. */
    ...
    /* ИНИЦИАТИВА: +1 ЗА КАЖДЫЕ ПОЛНЫЕ 5 ЛОВКОСТИ, У ВСЕХ КЛАССОВ.

       Считается прямо от ловкости, а не по ступеням статов: ступени
       начинаются с десятки, и на старте инициативы не было ни у кого.

       Отдельного классового «+1 лучнику» больше нет — и не нужно: лучник
       начинает с ловкостью 5, значит единица достаётся ему сама. Правило
       одно на всех, а преимущество берётся из его же характеристик.
       Разбойник её не теряет: инициатива у него такая же, как у всех. */
...
function layerBase(acc, ctx) {
    ...
    if (ctx.action === 'spell') {
        /* СВОЙ УРОН У КАЖДОГО ЗАКЛИНАНИЯ. Здесь стояла шестёрка для всех
           без разбора: огненный шар и ледяной шип били одинаково, хотя у
           шипа удар слабее, зато он морозит. Число берём из самого
           заклинания, шестёрка остаётся запасной — для старых свитков,
           у которых поля ещё нет. */
...
function layerClass(acc, ctx) {
    ...
    /* БАРД НА ПУСТОМ НАСТРОЕНИИ ВСЁ РАВНО БЬЁТ.
       ...
       Теперь бьёт, но с −1 к броску. Отдельного правила про крит не нужно:
       поправка идёт в ИТОГ, а крит считается от итога — выпавшая двадцатка
       превращается в девятнадцать и до порога не достаёт.
...
function finalize(acc) {
    ...
    if (критМожно && acc.total >= acc.critThreshold) { outcome = 'crit';  mult = MULT.crit + acc.critPower; }
...
function derivePlayerEffects(player, world) {
    ...
    /* ГЛУХАЯ ОБОРОНА — поднятый щит воина. Гаснет сама по номеру раунда,
       ровно как песня: хранится только номер, снимать нечего.

       Заводится ЗДЕСЬ, а не рисуется отдельно каждым экраном: тогда щит
       сам появится кружком у игрока, значком у Ведущего и на стриме — с
       цифрой поправки. Поправка со знаком минус, потому что она достаётся
       ВРАГАМ: −5 к их броску по этому воину. */
    if (shieldActive({ shieldRound: num(player.shieldRound), round: round })) {
        buffs.push({
            id: 'shield', kind: 'buff', duration: 'round',
            name: 'Глухая оборона', icon: EFFECT_ICONS.shield,
            tooltip: 'Глухая оборона: враги бьют по нему с ' + SHIELD_PENALTY,
            modifiers: { atk: SHIELD_PENALTY }
        });
    }
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   Раз В БОЙ лучник стреляет ДВАЖДЫ подряд. Цель выбирается
   ...
   Держится на двух фактах: номер боя, в котором навык потратили, и
   сколько выстрелов осталось. Второй сгорает с концом боя, а не копится.
...
/* РАЗ В БОЙ, А НЕ РАЗ В ДЕНЬ.

   Было «раз в игровой день», и за вечер из трёх драк лучник пользовался
   навыком один раз — в первой же, — а дальше сидел с обычными выстрелами.
   День крутит мастер, и между боями он его не всегда двигает.

   Второе дыхание воина и воскрешение жреца ОСТАЮТСЯ раз в день: они
   вытаскивают из смерти, и по бою каждый было бы слишком.

   Отметка теперь хранит номер боя — тот же приём, что у Бадуна. Поле
   переименовано в `doubleShotBattle`: оставь прежнее имя со дня, и старая
   запись в базе читалась бы как номер боя. */
function canDoubleShot(who, world) {
    if (!who || who.cls !== 'archer') return false;
    if (isRogue(who)) return false;              /* разбойник стреляет иначе */
    return !usedThisBattle(who, 'doubleShotBattle', world);
}
...
function whyCantDoubleShot(who, world) {
    ...
    if (usedThisBattle(who, 'doubleShotBattle', world)) return 'В этом бою уже стреляли дважды';
    return '';
}

/* Факты: номер боя и сколько выстрелов осталось. Первый тратится сразу. */
function planDoubleShot(world) {
    return { doubleShotBattle: num((world || {}).battleNo), shotsLeft: DOUBLE_SHOT_SHOTS };
}
...
function resolveEnemyAttack(p) {
    /* Прикрытая щитом цель роняет бросок врага на пять. Поправка живёт
       ЗДЕСЬ, а не у зовущего: иначе дашборд и симулятор посчитают её
       по-разному, и замер разойдётся с игрой. */
    var roll = num(p.roll) + (p.targetShielded ? shieldPenalty(p.targetShield || {}) : 0);
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   ОТЛИЧИЕ ОТ ЦЕРКВИ — КТО ПЛАТИТ. Там всегда общак: поднять павшего дело
   партии. Здесь платит либо сам герой, либо общак — решает кнопка в окне, и
   у игрока, и у Ведущего. Поэтому кошелёк приходит числом, а не объектом:
   правилу всё равно, чей он.
   ══════════════════════════════════════════════════════════════════════ */
...
/* Что даёт ночёвка ОДНОМУ герою. Числа названы в самой главе, сцена 15:
   полное здоровье, полная мана, минус ступень опьянения. День крутит
   зовущий — он один на всю партию, и правилу о нём знать нечего. */
function planSleep(who) {
    var st = drunkStep(who);
    return {
        hp: num((who || {}).maxHP),
        mp: num((who || {}).maxMP),
        drunk: Math.max(0, st - 1)
    };
}
...
/* Воин выходит в бой с ПУСТОЙ шкалой и зарабатывает ярость сам.

   Пятёрка на старте была костылём: без неё воин падал раньше, чем копил, и
   оба навыка стоимостью 5 не доставались тем боям, ради которых придуманы.
   Костыль убран вместе с причиной — теперь у воина есть «Поднять щит»
   (ниже), которым он переживает первые раунды и копит под ударами.

   Замер при пустой шкале БЕЗ щита: тяжёлая проседала на 7-16 пунктов и
   четыре клетки из пяти уходили ниже коридора. Щит обязан это вернуть —
   если после замера не вернёт, поднимать надо его силу или откат, а не
   стартовую ярость обратно. */
var RAGE_ON_START = 0;
...
/* ══════════════════════════════════════════════════════════════════════
   ПОДНЯТЬ ЩИТ — классовый навык воина

   Даёт −5 к броску атаки ВСЕХ врагов по этому воину до конца ТЕКУЩЕГО
   раунда. Это ГЛАВНОЕ действие, как каст: подняв щит, воин в этом раунде
   не бьёт.

   Связке «выпил и окопался» это не мешает: зелье ход не тратит, оно занимает
   ВТОРОЕ действие (после него нельзя защищаться в этом раунде). Главное и
   второе — разные слоты, оба доступны в один ход.

   Ярости не стоит: при пустой шкале на старте платить ею нечем.

   ЧЕГО ЭТО СТОИТ ПО ЗАМЕРУ. Щит, забирающий ход, роняет тяжёлую сильнее,
   чем пустая шкала сама по себе: воин перестаёт бить, бой тянется, и за
   лишние раунды прилетает всей партии. Числа — в записке к сборке. Это
   решение автора, а не недосмотр.

   ПОЧЕМУ ТЕКУЩИЙ РАУНД, А НЕ СЛЕДУЮЩИЙ. Щит прикрывает только тех, кто
   ходит ПОСЛЕ воина, — и это решение игрока, а не лотерея: очередь у него
   перед глазами. Стоит трое волков после тебя — щит окупится. Стоишь
   последним — заряд уйдёт в пустоту, и клиент обязан кнопку погасить.

   ОТКАТ. Применил на раунде N — следующий раз на N+4. Три раунда паузы.
   Раз в бой не годится: игрок бережёт единственный заряд, идеальный момент
   не наступает, и бой кончается с неиспользованной кнопкой.

   ЧТО ЩИТ ДЕЛАЕТ С ЯРОСТЬЮ. Ярость капает от прошедшего урона, поэтому
   под щитом она копится медленнее — но ВЫГОДНЕЕ: замер на волке с атакой 4
   даёт 0,29 ярости за единицу здоровья без щита и 0,39 под щитом. Воин
   покупает ярость дешевле, просто дольше. Менять из-за щита правило ярости
   не потребовалось.

   ХРАНИТСЯ ОДИН ФАКТ — номер раунда, когда щит подняли. Активность и откат
   СЧИТАЮТСЯ сравнением с текущим раундом, как Присутствие и обморожение.
   Снимать нечего, значит нечего и забыть снять. */
/* ЩИТ ЗАВИСИТ ОТ ТОГО, ЧЕМ ВОИН ДЕРЖИТ УДАР.

   Со щитом в левой руке он закрывается всерьёз — −6. С двуручным оружием
   закрываться нечем, он уходит в глухую стойку — −3, и это осознанный
   размен: терпишь слабее, зато бьёшь сильнее. Одноручное без щита прикрыть
   нечем совсем — всего −2. */
...
var SHIELD_COOLDOWN = 3;    /* столько раундов пауза после применения */

function canRaiseShield(p) {
    if ((p || {}).cls !== 'warrior') return false;
    var было = num((p || {}).shieldRound);
    if (!было) return true;
    return num(p.round) >= было + SHIELD_COOLDOWN + 1;
}
...
function shieldCooldownLeft(p) {
    var было = num((p || {}).shieldRound);
    if (!было) return 0;
    return Math.max(0, было + SHIELD_COOLDOWN + 1 - num((p || {}).round));
}

function shieldActive(p) {
    return num((p || {}).shieldRound) > 0 && num(p.shieldRound) === num(p.round);
}

/* Есть ли кому достаться. Щит бесполезен, если после воина в очереди не
   осталось ни одного живого врага — тогда кнопку гасим и говорим почему. */
...
function whyCantRaiseShield(p, queue) {
    ...
    if (queue && !shieldWouldCover(queue, p.id)) {
        return 'После вас никто из врагов не ходит — щит не успеет пригодиться';
    }
    return '';
}
...
global.CombatRules = {
    ...
    shieldPenalty: shieldPenalty,
    canRaiseShield: canRaiseShield, shieldCooldownLeft: shieldCooldownLeft,
    shieldActive: shieldActive, shieldWouldCover: shieldWouldCover,
    ...
    TITLES: TITLES, titleFor: titleFor, nextTitle: nextTitle, mercLevel: mercLevel, hasItem: hasItem,
